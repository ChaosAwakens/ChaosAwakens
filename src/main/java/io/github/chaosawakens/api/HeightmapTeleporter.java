...
public class HeightmapTeleporter implements ITeleporter {
	@Nullable
	public PortalInfo getPortalInfo(Entity entity, ServerWorld targetWorld,
		...
		double coordDiff = DimensionType.getTeleportationScale(entity.level.dimensionType(),
		...
		Vector3d newPosVector = new Vector3d(
				MathHelper.clamp(entity.getX() * coordDiff * 1.0, Math.max(-2.9999872E7D, border.getMinX() + 16.0D),
						Math.min(2.9999872E7D, border.getMaxX() - 16.0D)),
				entity.getY(),
				MathHelper.clamp(entity.getZ() * coordDiff * 1.0, Math.max(-2.9999872E7D, border.getMinZ() + 16.0D),
						Math.min(2.9999872E7D, border.getMaxZ() - 16.0D)));
		...
		int tries = 0;
		do {
			targetPos = new BlockPos(newPosVector.x(),
					targetWorld.getHeight(Heightmap.Type.WORLD_SURFACE, (int) newPosVector.x(), (int) newPosVector.z()),
					newPosVector.z());
		...
		if (entity.getY() <= -1) {
			...
			entity.setPos(targetPos.getX(),
					targetWorld.getHeight(Heightmap.Type.WORLD_SURFACE, (int) newPosVector.x(), (int) newPosVector.z()),
					targetPos.getZ());
		}

		return new PortalInfo(new Vector3d(newPosVector.x(), targetPos.getY(), newPosVector.z()), new Vector3d(0, 0, 0),
