...
public class HeightmapTeleporter implements ITeleporter {
	...
	public PortalInfo getPortalInfo(Entity entity, ServerWorld targetWorld, Function<ServerWorld, PortalInfo> defaultPortalInfo) {
		...
		*/
		...
		BlockPos targetPos;
		do {
			targetPos = new BlockPos(entityPos.getX(), targetWorld.getHeight(Heightmap.Type.WORLD_SURFACE, entityPos.getX(), entityPos.getZ()), entityPos.getZ());
		} while(targetPos.getZ() == 0);
		...
		ChaosAwakens.debug("TELEPORTER", blockOffset.getX()+" "+entity.getPosY()+" "+result.startPos+" "+result.height);
		
		return new PortalInfo(new Vector3d(targetPos.getX() + blockOffset.getX(), targetPos.getY(), targetPos.getZ() + blockOffset.getZ()), motion, entity.rotationYaw, entity.rotationPitch);
	}
