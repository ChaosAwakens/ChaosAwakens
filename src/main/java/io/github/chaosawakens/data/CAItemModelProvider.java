...
public class CAItemModelProvider extends ItemModelProvider {
	...
	private void generate(final Collection<RegistryObject<Item>> items) {
		...
		final ResourceLocation defaultRL = new ResourceLocation("def");
		...
		for (RegistryObject<Item> item : items) {
			...
			if (item.getId().getPath().contains("critter_cage")) {
				getBuilder(item.getId().getPath()).parent(parentGenerated)
						.texture("layer0",
								ItemModelProvider.ITEM_FOLDER + "/critter_cages/"
										+ name.replaceAll("_critter_cage", ""))
						.override().predicate(appleCowRL, 1.0F).model(appleCow).end().override()
						.predicate(defaultRL, 1.0F).model(defaultt).end().override().predicate(beeRL, 1.0F).model(bee)
						.end().override().predicate(birdRL, 1.0F).model(bird).end().override()
						.predicate(carrotPigRL, 1.0F).model(carrotPig).end().override().predicate(catRL, 1.0F)
						.model(cat).end().override().predicate(caveSpiderRL, 1.0F).model(caveSpider).end().override()
						.predicate(cowRL, 1.0F).model(cow).end().override().predicate(creeperRL, 1.0F).model(creeper)
						.end().override().predicate(donkeyRL, 1.0F).model(donkey).end().override()
						.predicate(drownedRL, 1.0F).model(drowned).end().override().predicate(endermanRL, 1.0F)
						.model(enderman).end().override().predicate(foxRL, 1.0F).model(fox).end().override()
						.predicate(horseRL, 1.0F).model(horse).end().override().predicate(huskRL, 1.0F).model(husk)
						.end().override().predicate(llamaRL, 1.0F).model(llama).end().override()
						.predicate(mooshroomRL, 1.0F).model(mooshroom).end().override().predicate(ostrichRL, 1.0F)
						.model(ostrich).end().override().predicate(pandaRL, 1.0F).model(panda).end().override()
						.predicate(pigRL, 1.0F).model(pig).end().override().predicate(piraporuRL, 1.0F).model(piraporu)
						.end().override().predicate(polarBearRL, 1.0F).model(polarBear).end().override()
						.predicate(rabbitRL, 1.0F).model(rabbit).end().override().predicate(sheepRL, 1.0F).model(sheep)
						.end().override().predicate(skeletonRL, 1.0F).model(skeleton).end().override()
						.predicate(slimeRL, 1.0F).model(slime).end().override().predicate(spiderRL, 1.0F).model(spider)
						.end().override().predicate(strayRL, 1.0F).model(stray).end().override().predicate(wolfRL, 1.0F)
						.model(wolf).end().override().predicate(zombieVillagerRL, 1.0F).model(zombieVillager).end()
						.override().predicate(zombieRL, 1.0F).model(zombie).end();
			} else if (item.getId().getPath().contains("_spawn_egg")) {
				getBuilder(item.getId().getPath()).parent(parentGenerated).texture("layer0",
						ItemModelProvider.ITEM_FOLDER + "/spawn_eggs/" + name.replaceAll("_spawn_egg", ""));
			} else {
				if (!existingFileHelper.exists(getItemResourceLocation(name), TEXTURE)
						|| existingFileHelper.exists(getItemResourceLocation(name), MODEL))
					continue;
				getBuilder(item.getId().getPath())
						.parent(item.get().getMaxDamage(ItemStack.EMPTY) > 0 && !(item.get() instanceof ArmorItem)
								? parentHandheld
								: parentGenerated)
	...
	private void generateBlockItems(final Collection<RegistryObject<Item>> itemBlocks) {
		for (RegistryObject<Item> item : itemBlocks) {
			...
			if (item.getId().getPath().contains("_wall")) {
				...
				if (!existingFileHelper.exists(getBlockResourceLocation(name), MODEL)
						|| existingFileHelper.exists(getItemResourceLocation(name), MODEL))
					continue;
