...
public class MiningAlgorithm {
	...
	public void findBlocks() {
		...
		while (!blocksToBreak.equals(dummyBlocks)) {
			...
			blocksToBreak.removeIf(
					p -> !world.getBlockState(startingBlock).getBlock().equals(world.getBlockState(p).getBlock()));
	...
	public boolean tryBreak(BlockPos p) {
		...
		if (!world.isClientSide()) {
			...
			if (!block.removedByPlayer(state, world, p, player, !player.isCreative(), state.getFluidState()))
				return false;
			// block.playerDestroy(world, player, p, state, world.getBlockEntity(p),
	...
	private void addToDropsList(BlockPos p, Block block, BlockState state) {
		List<ItemStack> drops = Block.getDrops(state, (ServerWorld) world, p, world.getBlockEntity(p), player,
				player.getMainHandItem());
		...
		if (!world.isClientSide() && world.getGameRules().getBoolean(GameRules.RULE_DOBLOCKDROPS)
			...
			for (ItemStack item : itemsToDrop) {
				if (!player.inventory.add(item)) {
					ItemEntity itemEntity = new ItemEntity(world, playerPos.getX() + 0.5, playerPos.getY() + 0.5,
							playerPos.getZ() + 0.5, item);
	...
	public void mine() {
				...
				if (!world.isClientSide()) {
					...
					if (mh.isDamageableItem() && !player.isCreative()) {
						if (mh.getDamageValue() < mh.getMaxDamage()) {
							...
							mh.hurtAndBreak(1, player,
									player -> player.broadcastBreakEvent(EquipmentSlotType.MAINHAND));
							break;
						}
					}
				}
		...
		if (!world.isClientSide())
			dropItems();
		// world.getBlockState(startingBlock).getBlock().dropXpOnBlockBreak(world,
		// playerPos, totalXp);
