...
public class MesozoicVinesTreeDecorator extends TreeDecorator {
	public static final Codec<MesozoicVinesTreeDecorator> CODEC = RecordCodecBuilder.create((decorator) ->
		decorator.group(BlockState.CODEC.fieldOf("vine").forGetter((instance) -> instance.vine))
		.apply(decorator, MesozoicVinesTreeDecorator::new));
	private final BlockState vine;
	
	public MesozoicVinesTreeDecorator(BlockState vine) {
		this.vine = vine;
	}
	
	@Override
	protected TreeDecoratorType<?> type() {
		return CATreeDecoratorTypes.MESOZOIC_VINES_TREE_DECORATOR.get();
	}
