...
public class WaspNestStructure extends Structure<NoFeatureConfig> {
    protected final String path;
    private final BlockPos offset;

    public WaspNestStructure(Codec<NoFeatureConfig> codec, String path, BlockPos offset) {
        super(codec);
        this.path = path;
        this.offset = offset;
    }

    public WaspNestStructure(Codec<NoFeatureConfig> codec, String path) {
        this(codec, path, new BlockPos(0, 0, 0).immutable());
    }

    @Override
    public IStartFactory<NoFeatureConfig> getStartFactory() {
        return WaspNestStructure.Start::new;
    }
    ...
    public class Start extends StructureStart<NoFeatureConfig> {
        ...
        public void generatePieces(DynamicRegistries dynamicRegistryManager, ChunkGenerator chunkGenerator, TemplateManager templateManagerIn, int chunkX, int chunkZ, Biome biomeIn, NoFeatureConfig config) {
            int x = (chunkX << 4) + 7;
            int z = (chunkZ << 4) + 7;
            boolean netherFlag = biomeIn.getBiomeCategory() == Biome.Category.NETHER;
            ...
            JigsawManager.addPieces(dynamicRegistryManager,
                    ...
                    AbstractVillagePiece::new, chunkGenerator, templateManagerIn, netherFlag ? WaspNestStructure.getGround(chunkGenerator, chunkGenerator.getBaseColumn((chunkX << 4) + 7, (chunkZ << 4) + 7), chunkX, chunkZ) : blockpos, pieces, random, false, !netherFlag);
