		...
		if (this.nibble > 0) {
			...
			if (this.timeUntilHooked > 0) {
				...
				if (serverworld.getBlockState(new BlockPos((int)d0, (int)d1 - 1, (int)d2)).getMaterial() == net.minecraft.block.material.Material.WATER) {
					if (this.random.nextFloat() < 0.15F) {
						serverworld.sendParticles(this.fluid.is(FluidTags.WATER) ? ParticleTypes.BUBBLE : ParticleTypes.SPLASH, d0, d1 - (double)0.1F, d2, 1, f1, 0.1D, f2, 0.0D);
					}
					float f3 = f1 * 0.04F;
					float f4 = f2 * 0.04F;
					...
					if (this.random.nextFloat() < 0.15F) {
						serverworld.sendParticles(this.fluid.is(FluidTags.LAVA) ? ParticleTypes.SMOKE : ParticleTypes.DRIPPING_LAVA, d0, d1 - (double)0.1F, d2, 1, f1, 0.1D, f2, 0.0D);
					}
					float f3 = f1 * 0.04F;
					float f4 = f2 * 0.04F;
			...
			if (this.random.nextFloat() < f5) {
				...
				if (serverworld.getBlockState(new BlockPos(d4, d5 - 1.0D, d6)).getMaterial() == net.minecraft.block.material.Material.WATER) {
					...
					serverworld.sendParticles(ParticleTypes.LARGE_SMOKE, d4, d5, d6, 2 + this.random.nextInt(2), 0.1F, 0.0D, 0.1F, 0.0D);
				}
			}
