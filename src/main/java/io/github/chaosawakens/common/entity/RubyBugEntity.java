...
public class RubyBugEntity extends AnimalEntity implements IAnimatable {
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		if (event.isMoving()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ruby_bug.walk", true));
			return PlayState.CONTINUE;
		}
		
		event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ruby_bug.stand", true));
		return PlayState.CONTINUE;
	}
