...
//        attacker.setPunching(false);
	...
	/*
	 * public boolean canPunchTarget(Entity target) { if
	 * (!roboPounder.isAggressive()) return false; if (!roboPounder.canSee(target))
	 * return false;
	 * 
	 * Vector3d attackerVector = new Vector3d(roboPounder.getX(),
	 * roboPounder.getEyeY(), roboPounder.getZ()); Vector3d targetVector = new
	 * Vector3d(target.getX(), target.getEyeY(), target.getZ());
	 * 
	 * if (target.level != roboPounder.level ||
	 * targetVector.distanceToSqr(attackerVector) > 128.0D * 128.0D) return false;
	 * 
	 * return roboPounder.level.clip(new RayTraceContext(attackerVector,
	 * targetVector, RayTraceContext.BlockMode.COLLIDER,
	 * RayTraceContext.FluidMode.NONE, roboPounder)).getType() ==
