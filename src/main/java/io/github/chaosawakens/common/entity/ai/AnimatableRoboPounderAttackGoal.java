...
//	protected double distanceToTarget = getDistanceBetweenEntities(this.roboPounder, target);
    ...
    public boolean canPunch() {
    	return roboPounder.getSideSweep() == false && roboPounder.getRageRunning() == false && roboPounder.getRageMode() == false && canHit();
    }
    
    public boolean canSideSweep() {
    	return roboPounder.getPunching() == false && roboPounder.getRageRunning() == false && roboPounder.getRageMode() == false && canHit();
    }
    
    public boolean canRageRun() {
    	return roboPounder.getPunching() == false && roboPounder.getSideSweep() == false && enableRageMode() && canHit();
    }
...
/*	@Override
	public void tick() {
		super.tick();
		++this.checkInterval;
		if (this.checkInterval >= 0 && target != null && this.roboPounder != null) {
			@SuppressWarnings("unused")
			LivingEntity target2 = this.roboPounder.getTarget();
			target2 = target;
			//this.roboPounder.setAggressive(true);
			target = this.roboPounder.getTarget();
			boolean hasTarget = target != null;
			boolean isValidForRage = getDistanceBetweenEntities(roboPounder, target) >= 10.0D || getDistanceBetweenEntities(this.roboPounder, target) <= 6.0D && target != null;
			boolean isAggressive = this.roboPounder.isAggressive();
			boolean isValidForFirstPunch = getDistanceBetweenEntities(roboPounder, target) <= 2.0D && target != null && target.getHealth() <= 5.0F;
			boolean isValidForSecondPunch = getDistanceBetweenEntities(roboPounder, target) <= 4.0D && this.target != null;
			double maxStopTime = 2.0D;
			this.roboPounder.setRageMode(false);
			this.roboPounder.setRageRunning(false);
			isAggressive = false;
			assert target != null;
			...
			if (hasTarget) {
				//Rage attack, including start/stop animations
				if (isValidForRage) {
					this.roboPounder.setRageMode(true);
					
					for (double stopTime = 0.0D; stopTime < maxStopTime; stopTime++) {
						this.roboPounder.setSpeed(0.0F);	
						this.roboPounder.setMoving(false);
						this.roboPounder.setRageMode(true);
					}
					
					if (this.roboPounder.getSpeed() == 0.0F) {
						this.roboPounder.setSpeed((float) ((float) this.roboPounder.getAttributeValue(Attributes.MOVEMENT_SPEED) * 2.0D));
					}
					...
					this.roboPounder.moveTo(this.roboPounder.getTarget().getX(), this.roboPounder.getTarget().getY(), this.roboPounder.getTarget().getZ());
					this.roboPounder.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(this.roboPounder.getAttributeValue(Attributes.ATTACK_DAMAGE) * 2.0D);
					this.roboPounder.getAttribute(Attributes.ATTACK_KNOCKBACK).setBaseValue(this.roboPounder.getAttributeValue(Attributes.ATTACK_KNOCKBACK) * 2.0D);
					this.roboPounder.getAttribute(Attributes.ARMOR).setBaseValue(this.roboPounder.getAttributeValue(Attributes.ARMOR) * 2.0D);
					this.roboPounder.getAttribute(Attributes.KNOCKBACK_RESISTANCE).setBaseValue(this.roboPounder.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE) * 2.0D);
				} else {
					assert target != null;
					this.roboPounder.lookAt(this.roboPounder.getTarget(), 1.0F, 1.0F);
					isAggressive = false;
