...
public class ThunderStaffItem extends Item {
	...
	public ActionResult<ItemStack> use(World worldIn, PlayerEntity playerIn, Hand handIn) {
		...
		float xA = -MathHelper.sin(playerIn.yHeadRot * ((float) Math.PI / 180F))
				* MathHelper.cos(playerIn.xRot * ((float) Math.PI / 180F));
		float yA = -MathHelper.sin(playerIn.xRot * ((float) Math.PI / 180F));
		float zA = MathHelper.cos(playerIn.yHeadRot * ((float) Math.PI / 180F))
				* MathHelper.cos(playerIn.xRot * ((float) Math.PI / 180F));
		...
		worldIn.playSound(null, playerIn.getX(), playerIn.getY(), playerIn.getZ(), SoundEvents.ARROW_SHOOT,
				SoundCategory.PLAYERS, 1.0F, 1.0F / (random.nextFloat() * 0.4F + 1.2F) + 0.5F);
