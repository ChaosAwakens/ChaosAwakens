...
public class MinersDreamItem extends Item {
	...
	public ActionResultType onItemUse(ItemUseContext context) {
		...
		int conX = pos.getX(), conY = (int) playerIn.getPosY(), conZ = conX = pos.getZ();
		...
		if (!world.isRemote) {
			world.playSound(playerIn, playerIn.getPosition(), SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.PLAYERS, 1.0F, 1.5F);
			...
			for (int l = 0; l < size[2]; l += this.torchDistance) {
				...
				if (!world.getBlockState(pos4).getBlock().isIn(CATags.AIR_BLOCKS) && world.getBlockState(pos3).getBlock().isIn(CATags.AIR_BLOCKS)) world.setBlockState(pos3, torchBlock);
			}
