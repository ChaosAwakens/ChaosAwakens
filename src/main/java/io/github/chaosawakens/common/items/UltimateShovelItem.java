...
public class UltimateShovelItem extends EnchantedShovelItem {
	...
	public UltimateShovelItem(IItemTier tier, int attackDamageIn, float attackSpeedIn, Properties builderIn,
	...
	public ActionResultType useOn(ItemUseContext ctx) {
		...
		if (ctx.getClickedFace() == Direction.DOWN) {
			...
			if (!world.isClientSide && playerentity != null) {
				ctx.getItemInHand().hurtAndBreak(1, playerentity,
						(p_220041_1_) -> p_220041_1_.broadcastBreakEvent(ctx.getHand()));
			}
			for (int x = -1; x < 2; x++) {
				for (int y = -1; y < 2; y++) {
					for (int z = -1; z < 2; z++) {
						BlockPos targetPos = new BlockPos(eventPos.getX() + x, eventPos.getY() + y,
								eventPos.getZ() + z);
						BlockState blockstate1 = blockstate.getToolModifiedState(world, targetPos, playerentity,
								ctx.getItemInHand(), net.minecraftforge.common.ToolType.SHOVEL);
						BlockState blockstate2 = null;
						if (world.isEmptyBlock(targetPos.above())) {
							if (world.getBlockState(targetPos).is(blockstate.getBlock())) {
								if (blockstate1 != null) {
									world.playSound(playerentity, targetPos, SoundEvents.SHOVEL_FLATTEN,
