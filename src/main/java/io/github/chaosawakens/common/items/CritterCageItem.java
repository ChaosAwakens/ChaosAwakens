...
<<<<<<< HEAD
	...
	/*
	 * protected static List<EntityType<?>> critterEntityBlacklist = new
	 * ArrayList<>(); //Type t; Properties p;
	 * 
	 * public CritterCageItem(Properties p) { super(p); this.p = p; // assert t !=
	 * null; }
	 * 
	 * public static boolean canUseCritterCage(EntityType<?> entityToCage, ItemStack
	 * stack) { return !(critterEntityBlacklist.contains(entityToCage)) &&
	 * !itemHasCritterCageMobAssigned(stack); }
	 * 
	 ...
	 * critterEntityBlacklist.add(EntityType.WITHER); return critterEntityBlacklist;
	 * }
	 * 
	 * //TODO Make this method and others similar to it more efficient/less clunky
	 * 
	 * @Override public List<EntityType<? extends LivingEntity>>
	 * addChaosEntityToCAEntityList(EntityType<? extends LivingEntity> CAMobToAdd) {
	 ...
	 * CHAOS_AWAKENS_ENTITIES.add(CAEntityTypes.WOOD_FISH.get()); return
	 * IUtilityHelper.super.addChaosEntityToCAEntityList(CAMobToAdd); }
	 * 
	 * @Override public List<EntityType<? extends LivingEntity>>
	 * addVanillaEntityToVanillaEntityList(EntityType<? extends LivingEntity>
	 * VanillaMobToAdd) { VANILLA_ENTITIES.add(EntityType.BAT);
	 * VANILLA_ENTITIES.add(EntityType.BEE); VANILLA_ENTITIES.add(EntityType.BLAZE);
	 * VANILLA_ENTITIES.add(EntityType.CAT);
	 * VANILLA_ENTITIES.add(EntityType.CAVE_SPIDER);
	 * VANILLA_ENTITIES.add(EntityType.CHICKEN);
	 * VANILLA_ENTITIES.add(EntityType.COD); VANILLA_ENTITIES.add(EntityType.COW);
	 ...
	 * VANILLA_ENTITIES.add(EntityType.FOX); VANILLA_ENTITIES.add(EntityType.GHAST);
	 ...
	 * VANILLA_ENTITIES.add(EntityType.ZOMBIFIED_PIGLIN); return
	 * IUtilityHelper.super.addVanillaEntityToVanillaEntityList(VanillaMobToAdd); }
	 * 
	 * public static boolean itemHasCritterCageMobAssigned(ItemStack itemToCheck) {
	 * if (itemToCheck.getTag() != null) { return
	 * itemToCheck.getTag().contains("EntityDataFilled") || itemToCheck.getItem()
	 * instanceof CritterCageItem; } return false; }
	 * 
	 * public ItemStack createFilledCritterCageWithMob(ItemStack critterCage,
	 * ItemStack newCageWithMob, CompoundNBT nbtData, EntityType<?> mobToCage,
	 * PlayerEntity player, Hand hand, World world) { boolean instaBuild =
	 ...
	 * } else {
	 * 
	 * if (!instaBuild) { critterCage.shrink(1); }
	 * 
	 * if (critterCage.isEmpty()) { return newCageWithMob; } else {
	 * 
	 * if (!player.inventory.add(newCageWithMob)) { player.drop(newCageWithMob,
	 * false); }
	 * 
	 * if (newCageWithMob.getItem().getTags() == null) {
	 * newCageWithMob.getOrCreateTag().put("EntityDataFilled", nbtData); }
	 * 
	 * if (mobToCage == null) { return null; }
	 * 
	 * return critterCage; }
	 * 
	 * }
	 ...
	 * }
	 * 
	 * public ItemStack defaultToFilledCritterCageIfMobIsUnsupported(ItemStack
	 * mobToCageInStack, CompoundNBT nbtData, Hand hand, World world, LivingEntity
	 * entity) { getMatching(entity, mobToCageInStack, mobToCageInStack,
	 * mobToCageInStack, entity.getType().getRegistryName()); return
	 * mobToCageInStack; }
	 * 
	 * /* @Override public ActionResult<ItemStack> use(World w, PlayerEntity p, Hand
	 * h) {
	 * 
	 * RayTraceResult result = getPlayerPOVHitResult(w, p,
	 * RayTraceContext.FluidMode.SOURCE_ONLY); if (result.getType() ==
	...
	/*
	 * @Override public CompoundNBT getNBTDataFromLivingEntity(LivingEntity
	 * entityToGetNBTDataFrom) { CompoundNBT entityDataNBT = new CompoundNBT();
	 * entityDataNBT.putString("entity",
	 * entityToGetNBTDataFrom.getType().getRegistryName().toString());
	 * entityDataNBT.putString("id",
	 * EntityType.getKey(entityToGetNBTDataFrom.getType()).toString());
	 * entityDataNBT.putFloat("health", entityToGetNBTDataFrom.getHealth());
	 * entityDataNBT.putString("EntityDataFilled",
	 * entityToGetNBTDataFrom.getEntityData().toString());
	 * entityToGetNBTDataFrom.addAdditionalSaveData(entityDataNBT); return
	 * entityDataNBT; }
	 * 
	 * @SuppressWarnings({ "deprecation", "unused" }) public LivingEntity
	 * getEntityFromNBT(CompoundNBT nbt, World world, boolean withInfo) {
	 * LivingEntity entity = (LivingEntity) Registry.ENTITY_TYPE.get(new
	 * ResourceLocation(nbt.getString("entity"))).create(world); EntityType<?> eType
	 * = entity.getType(); assert getEntityFromNBT(nbt, world, withInfo) != null; if
	 * (withInfo) { entity.load(nbt); } return entity; }
	 * 
	 * 
	 * public LivingEntity getEntityFromStack(ItemStack stack, World world, boolean
	 * withInfo) { return
	 * getEntityFromNBT(stack.getOrCreateTag().getCompound("EntityDataFilled"),
	 * world, withInfo); }
	 * 
	 ...
	 * LivingEntity entity = getEntityFromNBT(compoundnbt, world, true); if
	 * (p_208076_1_ != null) { if (p_208076_1_ != null &&
	 * p_208076_1_.contains("EntityDataFilled", 4)) { if (compoundnbt.contains("id",
	 * 2)) { return
	 * EntityType.byString(compoundnbt.getString("id")).orElse(entity.getType()); }
	 * } } return entity.getType(); }
	 */

	/*
	 * @Override public ActionResult<ItemStack> use(World w, PlayerEntity p, Hand h)
	 * { final LivingEntity e = getEntityFromStack(p.getItemInHand(h), w, true);
	 * final ItemStack stackWithFilledMob = new ItemStack(new
	 * FilledCritterCageItem(this.p, e.getType())); final ItemStack s = new
	 * ItemStack(this); final Vector3d entityPos = new Vector3d(e.getX(), e.getY(),
	 * e.getZ());
	 * 
	 * final CompoundNBT entityFillingData = getNBTDataFromLivingEntity(e); // final
	 * EntityType<?> type = getType(stackWithFilledMob.getTag());
	 * 
	 * RayTraceResult raytraceresult = getPlayerPOVHitResult(w, p, FluidMode.ANY);
	 * 
	 * if (raytraceresult.getType() == RayTraceResult.Type.BLOCK) {
	 * p.displayClientMessage(new
	 * TranslationTextComponent("crittercage.clientmessage.mobisnullorerror.message"
	 * ), true); return ActionResult.fail(p.getItemInHand(h)); }else if (
	 * raytraceresult.getType() == RayTraceResult.Type.MISS) {
	 * p.displayClientMessage(new
	 * TranslationTextComponent("crittercage.clientmessage.mobisnullorerror.message"
	 * ), true); return ActionResult.fail(p.getItemInHand(h)); } else { if
	 * (e.isBaby()) { if (w.isClientSide) { addParticles(w, ParticleTypes.SMOKE,
	 * e.getX(), e.getY(), e.getZ(), e.getX() + 0.3D, e.getY() + 0.1D, e.getZ() +
	 * 0.2D, 10); w.playSound(p, entityPos.x ,entityPos.y, entityPos.z,
	 * SoundEvents.ARMOR_EQUIP_CHAIN, e.getSoundSource(), 1.0F, 1.0F); }
	 * createFilledCritterCageWithMob(s, stackWithFilledMob, entityFillingData, e,
	 * p, h, w); stackWithFilledMob.getOrCreateTag().put("EntityDataFilled",
	 * entityFillingData); if (stackWithFilledMob.getTag() == null) {
	 * stackWithFilledMob.setTag(entityFillingData); } e.remove(true); return
	 * ActionResult.success(p.getItemInHand(h)); } if (w.isClientSide) {
	 * addParticles(w, ParticleTypes.LARGE_SMOKE, e.getX(), e.getY(), e.getZ(),
	 * e.getX() + 0.3D, e.getY() + 0.1D, e.getZ() + 0.2D, 10); w.playSound(p,
	 * entityPos.x ,entityPos.y, entityPos.z, SoundEvents.ARMOR_EQUIP_NETHERITE,
	 * e.getSoundSource(), 1.0F, 1.0F); } createFilledCritterCageWithMob(s,
	 * stackWithFilledMob, entityFillingData, e, p, h, w);
	 * stackWithFilledMob.getOrCreateTag().put("EntityDataFilled",
	 * entityFillingData); if (stackWithFilledMob.getTag() == null) {
	 * stackWithFilledMob.setTag(entityFillingData); } e.remove(true); return
	 * ActionResult.success(p.getItemInHand(h)); } }
	 */

	/*
	 * @Override public ActionResultType interactLivingEntity(ItemStack s,
	 * PlayerEntity p, LivingEntity e, Hand h) {
	 * 
	 * final World playerLevel = p.level; final Vector3d entityPos = new
	 * Vector3d(e.getX(), e.getY(), e.getZ()); RayTraceResult raytraceresult =
	 * getPlayerPOVHitResult(p.level, p, FluidMode.SOURCE_ONLY);
	 * 
	 * final ItemStack stackWithFilledMob = new ItemStack(new
	 * FilledCritterCageItem(this.p, e.getType())); final CompoundNBT
	 * entityFillingData = getNBTDataFromLivingEntity(e); final EntityType<?> type =
	 * getType(stackWithFilledMob.getTag());
	 * 
	 * if (raytraceresult.getType() == RayTraceResult.Type.BLOCK ||
	 * raytraceresult.getType() == RayTraceResult.Type.MISS) {
	 * p.displayClientMessage(new
	 * TranslationTextComponent("crittercage.clientmessage.mobisnullorerror.message"
	 * ), true); return ActionResultType.FAIL; }
	 * 
	 * if (isInCritterCageCAEntityList(e.getType()) &&
	 * canUseCritterCage(e.getType(), stackWithFilledMob)) { if (e.isBaby()) { if
	 * (playerLevel.isClientSide) { addParticles(playerLevel, ParticleTypes.SMOKE,
	 * entityPos.x, entityPos.y, entityPos.z, entityPos.x + 0.3D, entityPos.y +
	 * 0.2D, entityPos.z + 0.3D, 10); playerLevel.playSound(p, entityPos.x,
	 * entityPos.y, entityPos.z, SoundEvents.ARMOR_EQUIP_IRON, e.getSoundSource(),
	 * 1.0F, 1.0F); } createFilledCritterCageWithMob(s, stackWithFilledMob,
	 * entityFillingData, e, p, h, playerLevel); if (entityFillingData != null) {
	 * e.addAdditionalSaveData(entityFillingData); } //
	 * entityFillingData.putString("EntityType", "ChaosAwakensEntity");
	 * stackWithFilledMob.getOrCreateTag().put("EntityDataFilled",
	 * entityFillingData); stackWithFilledMob.setTag(entityFillingData);
	 * e.remove(true); return ActionResultType.SUCCESS; } else { if
	 * (playerLevel.isClientSide) { addParticles(playerLevel,
	 * ParticleTypes.LARGE_SMOKE, entityPos.x, entityPos.y, entityPos.z, entityPos.x
	 * + 0.3D, entityPos.y + 0.2D, entityPos.z + 0.3D, 10); } else {
	 * createFilledCritterCageWithMob(s, stackWithFilledMob, entityFillingData, e,
	 * p, h, playerLevel); if (entityFillingData != null) {
	 * e.addAdditionalSaveData(entityFillingData); } //
	 * entityFillingData.putString("EntityType", "ChaosAwakensEntity");
	 * stackWithFilledMob.getOrCreateTag().put("EntityDataFilled",
	 * entityFillingData); stackWithFilledMob.setTag(entityFillingData);
	 * e.remove(true); playerLevel.playSound(p, entityPos.x, entityPos.y,
	 * entityPos.z, SoundEvents.ARMOR_EQUIP_NETHERITE, e.getSoundSource(), 1.0F,
	 * 1.0F); } } return ActionResultType.SUCCESS; }
	 * 
	 * else if (isInCritterCageVanillaEntityList(e.getType()) &&
	 * canUseCritterCage(e.getType(), stackWithFilledMob)) { if (e.isBaby()) { if
	 * (playerLevel.isClientSide) { createFilledCritterCageWithMob(s,
	 * stackWithFilledMob, entityFillingData, e, p, h, playerLevel); if
	 * (entityFillingData != null) { e.addAdditionalSaveData(entityFillingData); }
	 * // entityFillingData.putString("EntityType", "ChaosAwakensEntity");
	 * stackWithFilledMob.getOrCreateTag().put("EntityDataFilled",
	 * entityFillingData); stackWithFilledMob.setTag(entityFillingData);
	 * e.remove(true); playerLevel.playSound(p, entityPos.x, entityPos.y,
	 * entityPos.z, SoundEvents.ARMOR_EQUIP_IRON, e.getSoundSource(), 1.0F, 1.0F); }
	 * createFilledCritterCageWithMob(s, stackWithFilledMob, entityFillingData, e,
	 * p, h, playerLevel); return ActionResultType.SUCCESS; } else {
	 * createFilledCritterCageWithMob(s, stackWithFilledMob, entityFillingData, e,
	 * p, h, playerLevel); if (entityFillingData != null) {
	 * e.addAdditionalSaveData(entityFillingData); } //
	 * entityFillingData.putString("EntityType", "ChaosAwakensEntity");
	 ...
	 * 
	 * @Override public ActionResultType interactLivingEntity(ItemStack s,
	 * PlayerEntity p, LivingEntity e, Hand h) { CompoundNBT nbt =
	 * getNBTDataFromLivingEntity(e); EntityType<?> type = getType(nbt, e.level);
	 * ItemStack newStack = new ItemStack(new FilledCritterCageItem(this.p, type));
	 * if (e instanceof PlayerEntity || !e.isAlive() || !canUseCritterCage(type, s)
	 * || newStack == null || newStack.getItem() == Items.AIR) { return
