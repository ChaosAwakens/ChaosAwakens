...
public class EnchantedScytheItem extends ScytheItem implements IAutoEnchantable {
	...
	public EnchantedScytheItem(EnumUtil.CAItemTier tierIn, int attackDamageIn, float attackSpeedIn, double attackReachIn, double attackKnockbackIn, Properties builderIn, Supplier<EnchantmentData[]> enchantments) {
		super(tierIn, attackDamageIn, attackSpeedIn, attackReachIn, attackKnockbackIn, builderIn);
...
//          }
	...
	public void fillItemCategory(ItemGroup group, NonNullList<ItemStack> items) {
		if (this.allowdedIn(group)) {
			...
			if (CAConfigManager.MAIN_COMMON.enableAutoEnchanting.get()) for (EnchantmentData enchant : enchantments.get()) stack.enchant(enchant.enchantment, enchant.level);
			items.add(stack);
		}
	}
