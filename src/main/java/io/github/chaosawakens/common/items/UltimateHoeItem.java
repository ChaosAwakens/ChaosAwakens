...
public class UltimateHoeItem extends EnchantedHoeItem {
	...
	public UltimateHoeItem(IItemTier tier, int attackDamageIn, float attackSpeedIn, Properties builderIn,
	...
	public ActionResultType useOn(ItemUseContext context) {
		...
		if (context.getClickedFace() != Direction.DOWN && world.isEmptyBlock(eventPos.above())) {
			BlockState blockState = world.getBlockState(eventPos).getToolModifiedState(world, eventPos,
					context.getPlayer(), context.getItemInHand(), ToolType.HOE);
			if (blockState != null) {
				...
				if (!world.isClientSide && playerentity != null) {
					context.getItemInHand().hurtAndBreak(1, playerentity,
					...
					for (int y = -1; y < 2; y++) {
						for (int z = -1; z < 2; z++) {
							...
							if (world.isEmptyBlock(targetPos.above())) {
								blockState = world.getBlockState(targetPos).getToolModifiedState(world, targetPos,
										context.getPlayer(), context.getItemInHand(), ToolType.HOE);
								if (blockState != null) {
