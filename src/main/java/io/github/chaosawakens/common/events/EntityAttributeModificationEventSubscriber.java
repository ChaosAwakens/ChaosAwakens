...
//		event.add(EntityType.PLAYER, ForgeMod.REACH_DISTANCE.get(), ExtendedHitWeaponItem.reach);
	...
	public static class ItemAttributeModificationEventSubscriber {
		// Setting the reach attribute in the item's registry or class is useless, for
		// now
		@SubscribeEvent
		public static void onEntityAttributeModificationEvent(ItemAttributeModifierEvent event) {
			...
			if (stack.getItem() instanceof ExtendedHitWeaponItem && slotType == EquipmentSlotType.MAINHAND
					&& event.getOriginalModifiers() != null) {
				if (stack.getItem() == CAItems.BIG_BERTHA.get()) {
					event.addModifier(ForgeMod.REACH_DISTANCE.get(),
							new AttributeModifier(ExtendedHitWeaponItem.ATTACK_REACH_MODIFIER, "Weapon modifier", 4.5D,
									AttributeModifier.Operation.ADDITION));
				}

				if (stack.getItem() == CAItems.ROYAL_GUARDIAN_SWORD.get()) {
					event.addModifier(ForgeMod.REACH_DISTANCE.get(),
							new AttributeModifier(ExtendedHitWeaponItem.ATTACK_REACH_MODIFIER, "Weapon modifier", 5.0D,
									AttributeModifier.Operation.ADDITION));
				}

				if (stack.getItem() == CAItems.ATTITUDE_ADJUSTER.get()) {
					event.addModifier(ForgeMod.REACH_DISTANCE.get(),
							new AttributeModifier(ExtendedHitWeaponItem.ATTACK_REACH_MODIFIER, "Weapon modifier", 3.0D,
									AttributeModifier.Operation.ADDITION));
				}
			}

			if (stack.getItem() instanceof ExtendedHitAxeItem && slotType == EquipmentSlotType.MAINHAND
					&& event.getOriginalModifiers() != null) {
				if (stack.getItem() == CAItems.SLAYER_CHAINSAW.get()) {
					event.addModifier(ForgeMod.REACH_DISTANCE.get(),
							new AttributeModifier(ExtendedHitAxeItem.ATTACK_REACH_MODIFIER, "Weapon modifier", 2.5D,
									AttributeModifier.Operation.ADDITION));
				}

				if (stack.getItem() == CAItems.BATTLE_AXE.get()) {
					event.addModifier(ForgeMod.REACH_DISTANCE.get(),
							new AttributeModifier(ExtendedHitAxeItem.ATTACK_REACH_MODIFIER, "Weapon modifier", 3.5D,
									AttributeModifier.Operation.ADDITION));
				}

				if (stack.getItem() == CAItems.QUEEN_SCALE_BATTLE_AXE.get()) {
					event.addModifier(ForgeMod.REACH_DISTANCE.get(),
							new AttributeModifier(ExtendedHitAxeItem.ATTACK_REACH_MODIFIER, "Weapon modifier", 4.5D,
									AttributeModifier.Operation.ADDITION));
				}
			}
		}
