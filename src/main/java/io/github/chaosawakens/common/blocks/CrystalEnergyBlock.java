...
public class CrystalEnergyBlock extends CAOreBlock {
	...
	public void playerWillDestroy(World worldIn, BlockPos pos, BlockState state, PlayerEntity player) {
		if (!player.isCreative() && !worldIn.isClientSide && worldIn.random.nextInt(3) == 0)
			worldIn.explode(null, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, 1.5F, worldIn.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING) ? Explosion.Mode.DESTROY : Explosion.Mode.NONE);
