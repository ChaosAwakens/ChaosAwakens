...
public class CrystalEnergyBlock extends CAOreBlock {
	...
	public void animateTick(BlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		if (worldIn.random.nextInt(2) == 0) {
			worldIn.addParticle(EMIT_PARTICLES.get(worldIn.random.nextInt(EMIT_PARTICLES.size())), pos.getX() + worldIn.random.nextDouble(), pos.getY() + worldIn.random.nextDouble(), pos.getZ() + worldIn.random.nextDouble(), 0.0D, 0.0D, 0.0D);
		}
	}
}
