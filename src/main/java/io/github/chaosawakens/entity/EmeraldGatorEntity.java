...
public class EmeraldGatorEntity extends AnimalEntity implements IAngerable, IAnimatable {
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		ChaosAwakens.LOGGER.debug(event.isMoving());
	...
	public static AttributeModifierMap.MutableAttribute setCustomAttributes() {
		return MobEntity.registerAttributes().createMutableAttribute(Attributes.MAX_HEALTH, 18.0D).createMutableAttribute(Attributes.ARMOR, 3).createMutableAttribute(Attributes.ATTACK_DAMAGE, 3.0D).createMutableAttribute(Attributes.ATTACK_KNOCKBACK, 1.0D).createMutableAttribute(Attributes.ATTACK_SPEED, 1.0D).createMutableAttribute(Attributes.MOVEMENT_SPEED, 0.1F).createMutableAttribute(Attributes.FOLLOW_RANGE, 8);
	}
