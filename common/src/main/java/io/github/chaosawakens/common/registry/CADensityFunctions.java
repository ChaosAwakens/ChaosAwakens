...
public class CADensityFunctions { // Needed so that Vanilla datapacks don't directly affect CA dimension terrain gen (plus some other density functions or optimizations, probably)
    ...
    public static final Supplier<ResourceKey<DensityFunction>> MINING_PARADISE_FACTOR = registerDensityFunction("mining_paradise/land/factor", b -> () -> NoiseRouterData.splineWithBlending(DensityFunctions.spline(WorldGenUtil.miningParadiseFactor(new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, CONTINENTS)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, EROSION)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, RIDGES)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, RIDGES_FOLDED)))), DensityFunctions.constant(1D)));
