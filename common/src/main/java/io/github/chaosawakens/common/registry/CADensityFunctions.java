...
public class CADensityFunctions { // Needed so that Vanilla datapacks don't directly affect CA dimension terrain gen (plus some other density functions or optimizations, probably)
    ...
    public static final Supplier<ResourceKey<DensityFunction>> MINING_PARADISE_RIDGES = registerDensityFunction("mining_paradise/land/ridges", b -> () -> DensityFunctions.flatCache(DensityFunctions.shiftedNoise2d(getWrappedDensityFunctionHolder(b, SHIFT_X), getWrappedDensityFunctionHolder(b, SHIFT_Z), 0.35D, b.lookup(Registries.NOISE).getOrThrow(Noises.RIDGE))));
    public static final Supplier<ResourceKey<DensityFunction>> MINING_PARADISE_RIDGES_FOLDED = registerDensityFunction("mining_paradise/land/ridges_folded", b -> () -> DensityFunctions.mul(DensityFunctions.add(DensityFunctions.add(getWrappedDensityFunctionHolder(b, RIDGES).abs(), DensityFunctions.constant(-1.5D / 3.0D)).abs(), DensityFunctions.constant(-1.25D / 3.0D)), DensityFunctions.constant(-2.75D)));

    public static final Supplier<ResourceKey<DensityFunction>> MINING_PARADISE_OFFSET = registerDensityFunction("mining_paradise/land/offset", b -> () -> DensityFunctions.flatCache(DensityFunctions.cache2d(DensityFunctions.lerp(DensityFunctions.blendAlpha(), DensityFunctions.add(DensityFunctions.constant(1.0D), DensityFunctions.spline(WorldGenUtil.miningParadiseOffset(new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, CONTINENTS)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, EROSION)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, RIDGES_FOLDED))))), DensityFunctions.blendOffset()))));
    ...
    public static final Supplier<ResourceKey<DensityFunction>> MINING_PARADISE_FACTOR = registerDensityFunction("mining_paradise/land/factor", b -> () -> NoiseRouterData.splineWithBlending(DensityFunctions.spline(WorldGenUtil.miningParadiseFactor(new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, CONTINENTS)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, EROSION)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, RIDGES)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, RIDGES_FOLDED)))), DensityFunctions.constant(10D)));
    public static final Supplier<ResourceKey<DensityFunction>> MINING_PARADISE_DEPTH = registerDensityFunction("mining_paradise/land/depth", b -> () -> DensityFunctions.add(DensityFunctions.yClampedGradient(-256, 480, 3.0D, -7.0D), getWrappedDensityFunctionHolder(b, MINING_PARADISE_OFFSET)));
