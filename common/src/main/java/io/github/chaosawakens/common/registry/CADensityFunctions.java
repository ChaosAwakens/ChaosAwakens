...
public class CADensityFunctions { // Needed so that Vanilla datapacks don't directly affect CA dimension terrain gen (plus some other density functions or optimizations, probably)
    ...
    public static final Supplier<ResourceKey<DensityFunction>> MINING_PARADISE_FACTOR = registerDensityFunction("mining_paradise/land/factor", b -> () -> NoiseRouterData.splineWithBlending(DensityFunctions.spline(WorldGenUtil.miningParadiseFactor(new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, CONTINENTS)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, EROSION)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, RIDGES)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, RIDGES_FOLDED)))), DensityFunctions.constant(25.0D)));
    public static final Supplier<ResourceKey<DensityFunction>> MINING_PARADISE_DEPTH = registerDensityFunction("mining_paradise/land/depth", b -> () -> DensityFunctions.add(DensityFunctions.yClampedGradient(-256, 480, 25.0D, -25.0D), getWrappedDensityFunctionHolder(b, MINING_PARADISE_OFFSET)));
