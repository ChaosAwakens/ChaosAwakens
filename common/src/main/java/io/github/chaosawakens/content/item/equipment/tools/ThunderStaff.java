...
public class ThunderStaff extends Item {
    ...
    private static final float PROJECTILE_VELOCITY = 0.75F;
    private static final float PROJECTILE_INACCURACY = 2.0F;
    private static final float SHOOT_SOUND_VOLUME = 1.0F;
    private static final float SHOOT_SOUND_PITCH = 1.0F;

    public ThunderStaff(Properties builderIn) {
        super(builderIn);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide()) {
            ThunderStaffProjectileEntity projectile = createProjectile(level, player);
    ...
    private static ThunderStaffProjectileEntity createProjectile(Level level, Player player) {
        Vec3 lookAngle = player.getLookAngle();
        ThunderStaffProjectileEntity projectile = new ThunderStaffProjectileEntity(level, player, lookAngle.x, lookAngle.y, lookAngle.z);
