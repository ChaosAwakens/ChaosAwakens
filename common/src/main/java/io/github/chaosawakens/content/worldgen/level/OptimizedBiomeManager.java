package io.github.chaosawakens.content.worldgen.level;
 ...
 * <br></br>
 * Of course, surface rules are responsible for the bulk of lost cpu time. That's going to be optimized in parallel as
 * well.
 */
public class OptimizedBiomeManager extends BiomeManager {
    public static final double FIDDLE_MULTIPLIER = 0.9D / 1024.0D; // 0.00087890625
    public static final double FIDDLE_OFFSET = -0.45D; // -0.5 * 0.9
    ...
    public @NotNull Holder<Biome> getBiome(BlockPos targetPos) { // ZOOM isn't used since we unrolled the loop and took care of each grid corner ourselves anyway
        ...
        // Calculate chunk coordinates using bitwise shift (slightly faster than division)
        final int chunkX = x >> ZOOM_BITS;
        final int chunkY = y >> ZOOM_BITS;
        final int chunkZ = z >> ZOOM_BITS;

        // Calculate local positions within chunk (slightly faster than modulo)
        final double localX = (x & ZOOM_MASK) * 0.25D;  // 1.0/4.0 = 0.25 (we want 1/4th)
        final double localY = (y & ZOOM_MASK) * 0.25D;
        final double localZ = (z & ZOOM_MASK) * 0.25D;

        // Precompute chunk offsets and local position adjustments
        int bestIndex = 0;
        double minDistance = Double.POSITIVE_INFINITY;
        final long seed = biomeZoomSeed;

        // Unroll the loop for better performance (scraps add up tbf)
        ...
        // Corner 1: (0,0,1)
        double d1 = getFiddledDistance(seed, chunkX, chunkY, chunkZ + 1, localX, localY, localZ - 1.0D);
        ...
        // Corner 2: (0,1,0)
        double d2 = getFiddledDistance(seed, chunkX, chunkY + 1, chunkZ, localX, localY - 1.0D, localZ);
        ...
        // Corner 3: (0,1,1)
        double d3 = getFiddledDistance(seed, chunkX, chunkY + 1, chunkZ + 1, localX, localY - 1.0D, localZ - 1.0D);
        ...
        // Corner 4: (1,0,0)
        double d4 = getFiddledDistance(seed, chunkX + 1, chunkY, chunkZ, localX - 1.0D, localY, localZ);
        ...
        // Corner 5: (1,0,1)
        double d5 = getFiddledDistance(seed, chunkX + 1, chunkY, chunkZ + 1, localX - 1.0D, localY, localZ - 1.0D);
        ...
        // Corner 6: (1,1,0)
        double d6 = getFiddledDistance(seed, chunkX + 1, chunkY + 1, chunkZ, localX - 1.0D, localY - 1.0D, localZ);
        ...
        // Corner 7: (1,1,1)
        double d7 = getFiddledDistance(seed, chunkX + 1, chunkY + 1, chunkZ + 1, localX - 1.0D, localY - 1.0D, localZ - 1.0D);
