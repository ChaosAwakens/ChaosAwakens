...
public class StalagmiteFeature extends Feature<StalagmiteConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<StalagmiteConfiguration> context) {
        ...
        BlockState originState = config.block().getState(random, originPos);
        ...
        BlockPos.MutableBlockPos targetPos = new BlockPos.MutableBlockPos.MutableBlockPos(
                originPos.getX(),
                getLowestCorner(level, originPos, radius),
                originPos.getZ()
        );
            ...
            for (int x = -radius; x <= radius; x++) {
                for (int z = -radius; z <= radius; z++) {
                    ...
                    if (pillarCeiling >= -1 * smallerThanZeroFlag && pillarCeiling >= y) {
                        ...
                        level.setBlock(placePos, originState, 2);
                    }
                }
            }
        }
